...
const url = `http://localhost:8089/sanpham/filter/type=MH1&min=0&max=5000000&mau=all&kc=all&nh=all&name=`;
...
bt_Search.addEventListener('click', function () {
    ...
    const url = `http://localhost:8089/sanpham/filter/type=MH1&min=${minValue}&max=${maxValue}&mau=${selectedMauValues}&kc=${selectedKcValues}&nh=${selectedNhValues}&name=${search_content}`;
        ...
        .then(data => {
            ...
            data.data.forEach(product => {
                ...
                productItem.innerHTML = `
                  ...
                  <div class="main__products-item--price">
                    <label class="main__products-item--label-giakhuyenmai">${(1 - product.km.mucKM) * product.giaban}đ</label>
                    ${product.km.mucKM ? `<div class="main__products-item--label-giagoc"><label class="main__products-item--label-giagoc">${product.giaban}đ</label></div>` : ''}
    ...
    // }
    // else {
    //     fetch(`http://localhost:8089/sanpham/getProduct/${search_content}`)
    //         .then(response => response.json())
    //         .then(data => {
    //             //  console.log(data.data);
    //             // console.log(data.data.giaban);
    //             const productsList = document.getElementById('products-list');
    //             const productItem = document.createElement('li');
    //             productItem.classList.add('main__products-item');
    //             // console.log(product.hinhanh);
    //             productItem.innerHTML = `
    //           <div class="main__products-item--img" style="position: relative; background: url(assets/img/${data.data.hinhanh}); width: 290px; height: 290px; background-repeat: no-repeat; background-size: 100%;" onclick="redirectToDetailsPage('${data.data.maSP}')">
    //             <img src="./assets/img/logosanpham.png" alt="" style="position: absolute; top: 0px; left: 0px; width: 90px; height: 80px;">
    //             ${data.data.km.mucKM ? `<div class="main__products-item--discount"><label class="main__products-item--discount-label">${data.data.km.mucKM * 100}%</label></div>` : ''}
    //           </div>
    //           <div class="main__products-item--name">
    //             <label class="main__products-item--name-label">${data.data.tenSP}</label>
    //           </div>
    //           <div class="main__products-item--price">
    //             <label class="main__products-item--label-giakhuyenmai">${(1 - data.data.km.mucKM) * data.data.giaban}đ</label>
    //             <label class="main__products-item--label-giagoc">${data.data.giaban}đ</label>
    //           </div>
    //         `;
    //             //Sửa dòng 11 : width của background thành 290px;
    //             productsList.appendChild(productItem);
